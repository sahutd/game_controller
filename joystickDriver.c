...
static int pen_probe(struct usb_interface* interface, const struct usb_device_id* id)
{
    ...
    printk(KERN_INFO "[*]saimadhav joystick pluggedin %d %d \n", id->idVendor, id->idProduct);

    iface_desc = interface->cur_altsetting;
    printk(KERN_INFO "device is now getting probed \n");
    printk(KERN_INFO "NUMBER of endpoints is  %d\n", iface_desc->desc.bNumEndpoints);
    printk(KERN_INFO " interface class = %d\n", iface_desc->desc.bInterfaceClass);
    ...
    usb_fill_int_urb(
        my_urb, device, pipe, my_buffer, buffer_size, handler, NULL, endpoint->bInterval);
    usb_submit_urb(my_urb, GFP_ATOMIC);
    return 0;
}


static void pen_disco(struct usb_interface* interface)
{
    printk(KERN_INFO "[*]saimadhav joystick removed hahahahaha\n");
}


static struct usb_driver pen_driver = {
    .name = "ToshibaStick", .id_table = pen_table, .probe = pen_probe, .disconnect = pen_disco,
};
